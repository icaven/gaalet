...
int main()
{
   ...
   while(!viewer.done()) {
      ...
      auto G = eval(R_1_d*R_1_0*T_1*R_2_d*R_2_0*T_2-M_t);

      //Ball joint R_1:
      auto grad_S1_U = K_1*S_1 + lambda*G*M_t;

      S_1 = S_1 - alpha*grad_S1_U;

      //Ball joint R_2:
      auto grad_S2_U = K_2*S_2 + lambda*G*M_t;

      S_2 = S_2 - alpha*grad_S2_U;

      //Lambda
      lambda = lambda - eval(alpha*grade<0>(G*G));
