...
template<typename CL, typename M, typename T, unsigned int ID=0>
struct multivector : public expression<multivector<CL, M, T, ID> >
{
   static const unsigned int identifier = ID;
   ...
   void operator=(const expression<E>& e_) {
      ...
      element_t* temp_data = new element_t[size];
      ElementEvaluation<E>::eval(temp_data, e);
      delete [] data;
      data = temp_data;
   }
...
template<typename CL, typename M, typename T, unsigned int ID> T* multivector<CL, M, T, ID>::data = NULL;

template<typename CL, typename M, typename T>
struct multivector<CL, M, T, 0> : public expression<multivector<CL, M, T, 0> >
{
   static const unsigned int identifier = 0;
...
struct multivector<configuration_list<0x00, cl_null>, M, T, 0> : public expression<multivector<configuration_list<0x00, cl_null>, M, T, 0> >
{
...
GAALET_CUDA_HOST_DEVICE
gaalet::multivector<typename A::clist, typename A::metric, typename A::element_t, 0>
eval(const gaalet::expression<A>& a) {
   return gaalet::multivector<typename A::clist, typename A::metric, typename A::element_t, 0>(a);
}
